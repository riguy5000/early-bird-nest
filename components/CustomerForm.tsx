...
export function CustomerForm({ customer, isEditing = false, onSave, onCancel }: CustomerFormProps) {
  ...
  const validateForm = () => {
    ...
    if (!formData.placeId) newErrors.address = 'Please select an address from the suggestions';
    if (!formData.idType) newErrors.idType = 'ID type is required';
    if (!formData.idNumber.trim()) newErrors.idNumber = 'ID number is required';

    // Date of birth validation (must be 18+ for pawn transactions)
  ...
  const handlePlaceSelect = (placeDetails: any) => {
    setFormData(prev => ({
      ...
      zipCode: placeDetails.postalCode || prev.zipCode
    }));
    ...
    <Card className="w-full max-w-4xl">
      ...
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <Input
                    ...
                    onChange={(e) => {
                      const formatted = formatPhoneNumber(e.target.value);
                      setFormData(prev => ({ ...prev, altPhone: formatted }));
                    }}
                    className="pl-10"
                    maxLength={14}
                  />
          ...
          <div className="space-y-4">
            <h3 className="text-lg font-medium flex items-center gap-2">
              Identification (Required for Transactions)
            </h3>
