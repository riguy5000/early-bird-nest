...
export function CustomerDrawer({ isOpen, onClose, customer, onCustomerUpdate }: CustomerDrawerProps) {
  ...
  const skipBack = useCallback(() => {
    if (frontImage) {
      analyzeLicense(frontImage, null);
    }
  }, [frontImage]);
    ...
    try {
      ...
      if (data.image_quality !== 'clear') {
        setImageQualityWarning(
          data.image_quality_note || 'The front image appears blurry. Please verify the extracted information.'
        );
      }
    ...
    <Sheet open={isOpen} onOpenChange={onClose}>
      <SheetContent side="right" className="w-full sm:w-[440px] p-0 flex flex-col">
        <SheetHeader className="px-6 pt-6 pb-4 border-b">
          ...
          <SheetDescription>
            {editMode ? 'Enter customer details or scan their ID' : 'Customer details for this transaction'}
          </SheetDescription>
        </SheetHeader>
              ...
              <div className="space-y-3">
                {scanStep === 'idle' && (
                  <Button 
                    variant="outline" 
                    onClick={startScan}
                    className="w-full flex items-center gap-2 h-11 border-dashed border-2"
                  >
                ...
                {scanStep === 'front' && (
                  <div className="border-2 border-dashed border-primary/40 rounded-lg p-4 text-center space-y-3 bg-primary/5">
                    <Camera className="h-8 w-8 mx-auto text-primary" />
                    <div>
                      <p className="font-medium text-sm">Step 1: Front of License</p>
                      <p className="text-xs text-muted-foreground">Take a photo or upload the front side</p>
                    </div>
                    <Button onClick={() => frontInputRef.current?.click()} className="w-full">
                ...
                {scanStep === 'back' && (
                  <div className="border-2 border-dashed border-primary/40 rounded-lg p-4 text-center space-y-3 bg-primary/5">
                    ...
                    <Camera className="h-8 w-8 mx-auto text-primary" />
                    <div>
                      <p className="font-medium text-sm">Step 2: Back of License</p>
                      <p className="text-xs text-muted-foreground">Take a photo of the back side</p>
                    </div>
                    <Button onClick={() => backInputRef.current?.click()} className="w-full">
                      <Upload className="h-4 w-4 mr-2" />
                      Capture Back
                    </Button>
                    <Button variant="ghost" size="sm" onClick={skipBack} className="w-full text-xs">
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="col-span-2">
                  ...
                  {editMode ? (
                    <Input value={formData.name} onChange={(e) => updateField('name', e.target.value)} placeholder="Full name" className="mt-1" />
                  ) : (
                ...
                <div>
                  ...
                  {editMode ? (
                    <Input type="date" value={formData.dateOfBirth} onChange={(e) => updateField('dateOfBirth', e.target.value)} className="mt-1" />
                  ) : (
                ...
                <div className="col-span-2">
                  ...
                  {editMode ? (
                    <Input value={formData.licenseNumber} onChange={(e) => updateField('licenseNumber', e.target.value)} placeholder="License number" className="mt-1" />
                  ) : (
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-3">
                <div>
                  ...
                  {editMode ? (
                    <Input type="email" value={formData.email} onChange={(e) => updateField('email', e.target.value)} placeholder="customer@example.com" className="mt-1" />
                  ) : (
                    <div className="mt-1 text-sm flex items-center gap-2"><Mail className="h-3 w-3" />{customer?.email || 'Not provided'}</div>
                  )}
                </div>
                ...
                <div>
                  ...
                  {editMode ? (
                    <Input type="tel" value={formData.phone} onChange={(e) => updateField('phone', e.target.value)} placeholder="(555) 123-4567" className="mt-1" />
                  ) : (
                    <div className="mt-1 text-sm flex items-center gap-2"><Phone className="h-3 w-3" />{customer?.phone || 'Not provided'}</div>
                  )}
                </div>
                ...
                <div>
                  ...
                  {editMode ? (
                    <Textarea value={formData.address} onChange={(e) => updateField('address', e.target.value)} placeholder="Full address" rows={2} className="mt-1" />
                  ) : (
                    <div className="mt-1 text-sm flex items-start gap-2"><MapPin className="h-3 w-3 mt-0.5" /><span>{customer?.address || 'Not provided'}</span></div>
                  )}
                </div>
              </div>
            </div>
