                                ...
                                <div className="mt-2 pl-11 space-y-2">
                                  ...
                                  >
                                    ...
                                    {item.category !== 'Watch' && (item.metals || []).length > 1 && (() => {
                                      const additionalMetals = (item.metals || []).slice(1);
                                      const firstAdditionalMetal = additionalMetals[0];
                                      if (!firstAdditionalMetal) return null;
                                      ...
                                      return (
                                        ...
                                        >
                                          ...
                                          <MetalPuritySelect
                                            metal={firstAdditionalMetal.type}
                                            value={firstAdditionalMetal.karat}
                                            onChange={(v) => updateMetal(item.id, firstAdditionalMetal.id, { karat: v })}
                                            triggerClassName="w-[96px] h-10 text-[13px] bg-white border border-black/[0.06] rounded-[10px]"
                                          />
                                          <div className="flex items-center gap-1.5">
                                            <Input
                                              ref={(el) => weightInputRefs.current[`${item.id}_${firstAdditionalMetal.id}`] = el}
                                              type="text"
                                              inputMode="decimal"
                                              value={firstAdditionalMetal.weightRaw ?? (firstAdditionalMetal.weight || '')}
                                              onChange={(e) => {
                                                const value = e.target.value;
                                                if (value === '' || /^\d*\.?\d{0,2}$/.test(value)) {
                                                  const numValue = value === '' ? 0 : parseFloat(value) || 0;
                                                  updateMetal(item.id, firstAdditionalMetal.id, { weight: numValue, weightRaw: value });
                                                }
                                              }}
                                              onBlur={(e) => {
                                                const numValue = parseFloat(e.target.value) || 0;
                                                updateMetal(item.id, firstAdditionalMetal.id, { weight: numValue, weightRaw: undefined });
                                              }}
                                              onKeyDown={(e) => handleKeyPress(e, item.id, firstAdditionalMetal.id)}
                                              placeholder="0.00"
                                              className="w-20 h-10 text-[13px] bg-white border border-black/[0.06] rounded-[10px] text-right tabular-nums [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                                              style={{ MozAppearance: 'textfield' as any }}
                                              onClick={(e) => e.stopPropagation()}
                                          ...
                                          {store.canDeleteItems !== false && (
                                            <button
                                              onClick={(e) => {
                                                e.stopPropagation();
                                                const updatedMetals = (item.metals || []).filter((m: any) => m.id !== firstAdditionalMetal.id);
                                  ...
                                  {item.category !== 'Watch' && (item.metals || []).slice(2).map((metal: any) => (
                                    <div
                                      key={metal.id}
